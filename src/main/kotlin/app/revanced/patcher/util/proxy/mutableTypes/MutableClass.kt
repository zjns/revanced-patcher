...
class MutableClass(classDef: ClassDef) : ClassDef, BaseTypeReference() {
    ...
    private val _directMethods by lazy { _methods.filter { MethodUtil.isDirect(it) }.toMutableSet() }
    private val _virtualMethods by lazy { _methods.filter { !MethodUtil.isDirect(it) }.toMutableSet() }

    // Fields
    private val _fields by lazy { classDef.fields.map { field -> field.toMutable() }.toMutableSet() }
    private val _staticFields by lazy { _fields.filter { FieldUtil.isStatic(it) }.toMutableSet() }
    private val _instanceFields by lazy { _fields.filter { !FieldUtil.isStatic(it) }.toMutableSet() }
